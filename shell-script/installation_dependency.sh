...
installation_dependency(){
    if [[ ${release} == "centos" ]]; then
        yum -y update 
        yum install -y axel libncurses5-dev zlib1g-dev gawk flex patch git-core g++ subversion xz zip unzip make gcc wget python file
    else 
        if [[ ${release} == "ubuntu" || ${release} == "debian" ]]; then
            echo "installation_dependency"
          apt-get update -y
            apt-get install axel libncurses5-dev zlib1g-dev gawk flex patch git-core g++ subversion xz-utils zip unzip make gcc wget python file -y
        fi
    fi
}
