...
installation_dependency(){
    if [[ ${release} == "centos" ]]; then
        yum -y update 
        yum install -y axel libncurses5-dev zlib1g-dev gawk flex patch git-core g++ subversion xz zip unzip
    else 
        if [[ ${release} == "ubuntu" || ${release} == "debian" ]]; then
            echo "installation_dependency"
            apt-get update -y
            apt-get install axel libncurses5-dev zlib1g-dev gawk flex patch git-core g++ subversion xz-utils zip unzip -y
        fi
    fi
}
...
mv_images(){
    mkdir -p ${ZIP_DIR}
    mv ${BUILD_DIR}/openwrt-imagebuilder-${OPENWRT_VERSION}-${OPENWRT_PLATFORM}-${OPENWRT_TARGETS}.Linux-x86_64/${IMAGE_DIR}/* ${ZIP_DIR}
}
